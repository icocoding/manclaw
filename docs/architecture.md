...
- 配置校验
- 历史版本与回滚
- 模型与 Agent 的结构化表单编辑，并回写真实 `openclaw.json`
- Channels 的结构化实例编辑，并联动重建 `bindings[*].match`
- Agent 私有 skills 按 workspace 隔离：每个 agent 的本地 skills 来自该 workspace 下的 `skills/` 目录，而不是单独的 `agents.list[].skills` 配置字段
